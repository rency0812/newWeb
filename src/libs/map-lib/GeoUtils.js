 ...
 * 主入口类是<a href="/ExSun.GeoUtils.html">GeoUtils</a>，
 *
 *
 * @author ExSun Map Api Group
 * @version 2.4
 */

var ExSun = window.ExSun = ExSun || {};
...
(function() {
    ...
    /**
     * @exports GeoUtils as EX.GeoUtils
     */
    var GeoUtils =
        /**
    ...
    GeoUtils.isPointInCircle = function(point, circle) {
        //检查类型是否正确
        if (!(point instanceof L.Point) || !(circle instanceof L.Circle)) {
            return false;
        }
    ...
    GeoUtils.isPointOnPolyline = function(point, polyline){
        //检查类型
        if(!(point instanceof L.Point) ||
            !(polyline instanceof L.Polyline)){
            return false;
        }
    ...
    GeoUtils.getDistance = function(point1, point2){
        //判断类型
        if(!(point1 instanceof L.Point) ||
            !(point2 instanceof L.Point)){
            return 0;
        }
