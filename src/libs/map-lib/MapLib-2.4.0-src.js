...
L.MapLib.Measure.version = '2.4.0';
...
L.MapLib.LocaltionSearch.version = '2.4.0';
 ...
 */
L.MapLib.MapMaster = {};
L.MapLib.MapMaster.vertion = '2.4.0';
...
L.MapLib.MapControl.Layout = L.Evented.extend({
    ...
    initUI: function () {
        ...
        var el = L.create('div','ex-layout-map-content',this.pContainer);
        L.create('div','ex-layout-type-wbox ex-map-bottom ex-map-right',el);
        L.create('div','ex-layout-maptool ex-theme-maptool ex-map-top ex-map-left  ec-padding-0',el);
        L.create('div','ex-layout-maptool ex-theme-maptool ex-map-top ex-map-right',el);
        L.create('div','ex-layout-monitor-wbox',el);
...
L.MapLib.MapControl.ESMapEdit = L.Evented.extend({
    ...
    initUI: function () {
        ...
        var el=L.create('div','ex-maptool-box',this._pContainer);
        var ul =L.create('ul','ec-avg-sm-1 ex-map-tool-edit',el);
        this._createTool(ul);
    ...
    _createTool:function (ul) {
        ...
        var li = L.create('li', '', ul);
        this.btnPlus   = L.create('a','ec-icon-plus',li);
        this.btnPlus.text('&nbsp;&nbsp;绘制')

        li = L.create('li', '', ul);
        this.btnEdit = L.create('a','ec-icon-pencil',li);
        this.btnEdit.text('&nbsp;编&nbsp;&nbsp;辑')

        li = L.create('li', '', ul);
        this.btnCal = L.create('a','ec-icon-power-off',li);
        this.btnCal.text('&nbsp;删&nbsp;&nbsp;除');

        li = L.create('li', '', ul);
        this.btnSave =L.create('a','ec-icon-save',li);
        this.btnSave.text('&nbsp;确&nbsp;&nbsp;定')

        li = L.create('li', '', ul);
        this.btnDel =L.create('a','ex-map-tool-cancel',li);
...
L.MapLib.MapControl.ESMapFull = L.Evented.extend({
    ...
    initialize: function (oMapBase, options) {
        ...
        this.oPContainer = document.getElementsByClassName(this.options.acParentDivClass);
    ...
    //加载工具事件，初始化工具栏
    initUI: function () {
        this.container = L.create('div', 'ex-maptool-box ex-map-full', this.oPContainer)
        this.container.innerHTML = '&nbsp;&nbsp;全屏';
        L.create('i', 'ec-icon-expand', this.container)
        this.initToolEvent();
    },
...
L.MapLib.MapControl.ESMapTile = L.Evented.extend({
    ...
    initialize: function (oMapBase, options) {
        ...
        //初始化界面
        //this.container = L.create('div', 'ex-maptool-box ex-control-dropmenu', this.pContainer);

        // L.create('i', 'ec-icon-briefcase', this.container);
        // this.span = L.create('span', '', this.container);
        // this.span.innerHTML = '工具';
        // L.create('i', 'ec-icon-briefcase', this.container);
        // var ul = L.create('ul', 'ec-avg-sm-1 ec-dropdown-content', this.container);
        //
    ...
    initUI: function () {
        ...
        this.container = L.create('div', 'ex-maptool-box ex-control-dropmenu', this.pContainer);
        L.create('i', 'ec-icon-clone', this.container);
        this.span = L.create('span', '', this.container);
        this.span.innerHTML = '高德地图';
        L.create('i', 'ec-icon-angle-down', this.container);
        var ul = L.create('ul', 'ec-avg-sm-1 ec-dropdown-content', this.container);
    ...
    initToolEvent: function () {
        ...
        var li = L.create('li', '', ul);
        var a = L.create('a', '', li);
        a.innerHTML = '<i class="ex-icon-maptool ex-maptool-china">&nbsp;高德地图</i>';
        L.DomEvent.on(a, 'click', function (e) {
            var cName = e.innerText.trim();

            this.container.getElementsByTagName('span').html(cName);
            self.selectLayer(cName);
        }, this);

        L.DomEvent(this.container,'mouseover,mouseout',function (e) {
...
L.MapLib.MapControl.ESMapToolBox = L.Evented.extend({
    ...
    initToolEvent: function (ul) {
        ...
        L.DomEvent.on(a, 'click', function (e) {
            ...
            this.oActHandler = self.oDistantHandler;
        ...
        L.DomEvent.on(a, 'click', function (e) {
            ...
            this.oActHandler = self.oAreaHandler;
            this.oActHandler.enable();
        }, this);
...
L.MapLib.MapControl.ESMapSearch = L.Evented.extend({
    ...
    initUI: function () {
        ...
        this.container = L.create('div', 'ex-maptool-box', this.oPContainer)
        var el = L.create('div', 'ec-input-group ex-maptool-search-box', this.container);

        this.input = L.create('input', 'ec-form-field', el);
        var span = L.create('span', 'ec-input-group-btn', el);
        this.ul = L.create('ul', 'ex-maptool-box-search-result', el);


        this.search = L.create('button', 'ec-btn ec-btn-primary ec-btn-sm search', span);
        this.clear = L.create('button', 'ec-btn ec-btn-default ec-btn-sm clear', span);
        this.search.innerHTML = '<span class="ec-icon-search"></span>';
        this.clear.innerHTML = 'span class="ec-icon-close">';

        L.MapLib.hide(this.search);
    },
    ...
    initToolEvent: function () {
        ...
        L.DomEvent.on(document, 'keyup', function (e) {
            ...
            // 判断查询结果是否为上次的查询结果
            if (self.oInputData && self.oInputData.name === this.input.innerText) {
                return;
            }
            ...
            bTo = setTimeout(function () {
                ...
                var oParam = {};
                L.extend(oParam, self.oOption.oParam, {
                    keywords: cSearchVal,
                    location: oLatLng.lng + ',' + oLatLng.lat
                });
                L.getData(oParam, self.oOption.cUrl, self.searchPoiHandler, self);
            }, 250);
        }, this);
        ...
        L.DomEvent.on(this.search, 'click', function (e) {
            ...
            var cSearchVal = this.input.innerText;
            ...
            L.getData(oParam, this.options.cUrl, this.searchPoiHandler, this);
        }, this);
    ...
    localPos: function () {
        ...
        // 给文本框赋值
        this.input.innerText = oData.name;
    ...
    searchPoiHandler: function (oData) {
        ...
        for (var i = 0; i < oData.tips.length; i++) {
            ...
            var li = L.create('li', 'location', this.ul);
            li.innerHTML = L.Util.template(' <b>{name}</b><span>{cDist}</span> ', oData.tips[i]);
            li.oData = oData.tips[i];

            L.DomEvent.on(li, 'click', this, function (e) {
                e.data.localPos();
            }, this);

            L.DomEvent.on(li, 'mouseover', this, function (e) {
                e.addClass('ec-active');
            }, this);

            L.DomEvent.on(li, 'mouseout', this, function (e) {
                e.removeClass('ec-active');
            }, this);
        }
        L.MapLib.show(this.search);
    }
