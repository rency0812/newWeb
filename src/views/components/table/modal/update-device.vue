    ...
    export default {
        ...
        data() {
            return {
                ...
                saveBasicCfg: [
                    ...
                    {id: 2, type: 'select-device-type', label: '*设备类型', name: 'deviceTypeId', postUrl: '/table/deviceType/querySelectRemote'},
                    ...
                    {id: 8, type: 'input', label: '备注', name: 'remark'},
        ...
        methods: {
            ...
            handleChanelNumChange(v){
                ...
                // 如果原本有通道数据 则在此基础上额外加
                if(self.editModal.channelInfos.length > 0){
                    let len = self.editModal.channelInfos.length
                    self.pushChannelData(len, v)
                }else{
                    self.pushChannelData(0, v)
                }
            },
            pushChannelData(len, v){
                let self = this
                for(let i = len; i < v; i++){
                    self.chanelArr[i] = {
                        id: i+1,
            ...
            pushExsitChannelData(len, v){
                ...
                for(let i = len; i < v; i++){
                    self.chanelArr[i] = {
                        ...
                        cfg: [
                            {id: 1, type: 'input', label: '名称', name: 'channelName'},
                            {id: 2, type: 'input', label: '接入线路', name: 'channelLine'},
                            {id: 3, type: 'input', label: '安装说明', name: 'installInfo'},
                            {id: 4, type: 'input', label: '云台功能', name: 'cloudFunction'},
                            {id: 5, type: 'select-remote', label: '型号', name: 'modelNumber', dicType: 16}
                        ]
                    }
                }
            ...
            handleSaveDepartment(e) {
                ...
                if(this.rowData.channelNum){
                    this.editModal.channelNum = this.rowData.channelNum.toString()
                }
            ...
            handleSave() {
                ...
                this.editModal.channelNum = parseInt(this.editModal.channelNum)
