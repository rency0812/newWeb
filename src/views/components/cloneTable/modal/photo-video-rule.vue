  ...
  <div class="plus-rule-set">
    <Modal v-model="show" width="800" :draggable="false" :mask-closable="true" class="layou-modal-input">
      ...
      <Form :model="formData" class="top-form" label-position="right" :label-width="90">
        <h4 @click="isSpread(1)">{{ruleName}}<Icon style="float: right" size="30" :type="iType" /></h4>
        <Row v-show="topShow" class="top-box">
          <Col span="10" style="margin-right:120px;">
            ...
            <FormItem :class=" 'fix-time-'+i " v-for="(v, i) in fixTime" :key="i" :label="v.lab">
        ...
        <h4 @click="isSpread(2)" style="margin:0;">拍照录像设置<Icon style="float: right" size="30" :type="iType1" /></h4>
        <Row v-show="botShow" class="bot-box">
          <Col span="10" style="margin-right:120px;">
            <FormItem label="是否拍照">
              <RadioGroup @on-change="change" v-model="isPhoto">
                <Radio label="是"></Radio>
                <Radio label="否"></Radio>
              </RadioGroup>
            </FormItem>
            <section v-show="isPhoto.indexOf('是')>-1" class="pic-box1">
              <FormItem label="拍照时长" prop="">
                <div class="m-box" style="right:-18px;">
                  <p class="s1">s</p>
                  <p class="s2"></p>
                </div>
                <Input :size="inputSize" type="text" v-model="formData.photoTime" :placeholder="placeholder"></Input>
              </FormItem>
              ...
              <FormItem label="拍照摄像头" prop="">
                <Checkbox ref="pAll" @on-change="selectedAll($refs.pAll)" v-model="all">全部</Checkbox>
                <CheckboxGroup v-model="channel">
                  <Checkbox v-for="v in 16" :key="v" :label="v+'号'"></Checkbox>
                </CheckboxGroup>
              </FormItem>
            </section>
          </Col>
          <Col span="10">
            <FormItem label="是否录像">
              <RadioGroup v-model="isVideo">
                <Radio label="是"></Radio>
                <Radio label="否"></Radio>
              </RadioGroup>
            </FormItem>
            <section v-show="isVideo.indexOf('是')>-1" class="video-box1">
              ...
              <FormItem label="码流类型"  prop="">
                <Select :size="inputSize" v-model="formData.bitType" placeholder="请选择...">
                  <Option value="主码流">主码流</Option>
                  <Option value="子码流">子码流</Option>
                </Select>
              </FormItem>
              <FormItem label="录像摄像头"  prop="">
                <Checkbox ref="vAll" @on-change="selectedAll($refs.vAll)" v-model="all1">全部</Checkbox>
...
export default {
  ...
  methods:{
    ...
    selectedAll(type){
      let i = 1
      //console.log(type)
      if(type._uid === 695&&!type.value){
        while(i<17){
         this.channel.push(i+'号')
         i++
        }
      }else{
        this.channel=[]
      }
      if(type._uid === 734&&!type.value){
        while(i<17){
         this.video.push(i+'号')
         i++
        }
    ...
    getChannel(isAll,channelArr){
      ...
      if(isAll){
        ...
        channelArr.forEach(v => {
          arr.push(v.slice(0,-1))
        })
      }
      return arr
    },
    alertSet(){ //报警规则
      if(this.isPhoto.indexOf('是')>-1){
        this.formData.photo = true
        if(!this.formData.photoTime||!this.formData.photoInterval||!this.formData.photoCamera){
          this.$Message.info('请输入拍照相关参数')
          return 2
        }
      }else{
        this.formData.photo = false
        this.formData.photoTime = ''
        this.formData.photoCamera = ''
        this.formData.photoInterval = ''
      }
      if(this.isVideo.indexOf('是')>-1){
    ...
    add(i){
      ...
      const class1 = `.fix-time-${i+1}`
      const class2 = `.fix-time-${i} .add-remove`
      //if(i === 0){
        this.getClass(class1).style.display = 'block'
        this.getClass(class2).style.display = 'none'
      //}
      if(i === 2){
        this.getClass('.fix-time-3 .add-remove .p2').style.display = 'none'
      }
    },
    remove(i){
      const class1 = `.fix-time-${i}`
      const class2 = `.fix-time-${i-1} .add-remove`
...
<style lang="less">
  ...
  .fix-time-1,.fix-time-2,.fix-time-3{
    display: none;
  }
</style>
