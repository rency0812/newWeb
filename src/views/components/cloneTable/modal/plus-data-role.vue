<template>
  <div class="plus-func-role">
    ...
    <Modal v-model="show" width="610" :styles="{top: '2px'}" :draggable="false" :mask-closable="true" class="layou-modal-input">
      <p slot="header">
...
export default {
  ...
  methods: {
    ...
    save () {
      ...
      // }
      let nodeIds = this.$refs.role.roleData
      if(nodeIds.length){
        this.formData.deptIds = nodeIds
      }else{
        this.formData.deptIds = this.formData.deptIds||[]
      }
      util.ojax.post(url, {
        id: this.formData.id,
