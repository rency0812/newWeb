<template>
    <ul class="service-box">
        <li v-for="(item,index) in serviceData" :style="{width:liWidth,margin:'10px auto'}" :key="index">
            <a :href="item.url">
                <i class="ex-icon" :class="item.iconName"></i>
                <p>{{item.label}}</p>
            </a>
        </li>
        <li :style="{width:liWidth,margin:'10px auto'}" @click="exportsExcle">
