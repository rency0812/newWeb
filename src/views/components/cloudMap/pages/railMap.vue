...
import saveWnd from '../modal/save-rail'
    ...
    data(){
        return{
            ...
            mapControl:{
                ...
                mapContainerId:'railMap',
            },
            pageTree: [
                {'id': 1,'treeName': '组织机构', 'treeValue': 'departmentTree'},
                {'id': 2,'treeName': '围栏', 'treeValue': 'railTree'}
            ],
            buttonGroup: [
                {
                    id:1,
            ...
            tableColumns: [
                ...
                {title: '围栏名称', key: 'cloudName',align:'center'},
    ...
    methods:{
        ...
        initPageCfg(){
            ...
            Util.ojax.post(GetPageCfgUrl, {menuId: menuId, roleId: roleId}).then(function (res) {
                ...
                for(let i in cfg){
                    if(cfg[i].name == 'updateFence'){
                        self.monitorBtn.editBtn = true
                    }
                    if(cfg[i].name == 'insertFence'){
                        self.monitorBtn.saveBtn = true
                    }
                    if(cfg[i].name == 'deleteFence'){
        ...
        createdCallBack:function (e) {
            ...
            if(oLayer instanceof L.Circle) {
                ...
                this.aoLatLng = ao
                let cLat = this.getCenter(ao,'latitude')
                let cLon = this.getCenter(ao,'longitude')
                this.getCenterPoi(cLon + ',' + cLat) //转化实时地址
            }
            this.isShow = true
        ...
        initCloudMapTree(){
            let self = this
            Util.ojax.post(GetCloudMapTree, {cloudTypeId: 3}).then(function (res) {
                if (res.data.code == 0) {
