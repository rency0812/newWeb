        ...
        <Split v-model="panelSplit">
            <div slot="left" class="layout-tree-content">
                ...
                <div class="map-action-button">
                    <Button type="default" class="action-button" @click="handleClickItem" v-if="monitorBtn.saveBtn">新增</Button>
                    <Button type="default" class="action-button" @click="editCloudMap" v-if="monitorBtn.editBtn">编辑</Button>
                    <Button type="default" class="action-button" @click="deleteCloudMap" v-if="monitorBtn.deleteBtn">删除</Button>
                </div>
                <!--组织树-->
                <div style="margin-left: 30px" v-if="currentTree == 0">
                ...
                <div style="margin-left: 30px" v-else>
                    <div class="search-box" style="white-space: nowrap">
                        <Input v-model="tablePage.cloudName" size="small" autofocus enter-button
                               placeholder="请输入云图名称"
                               class="fl" style="max-width: 70%;"/>
                        <Button icon="md-refresh" size="small" style="margin: 1px 0 0 7px" @click="initCloudMapGrid">
                            查询
                        </Button>
                    </div>
                    <Table
                            ...
                            @on-select="onSelectGrid"
                            @on-select-all="onSelectGrid"
                            @on-select-cancel="onSelectCancelGrid"
                            @on-select-all-cancel="onSelectCancelGrid"></Table>
                </div>
...
import saveWnd from '../modal/save-point'
...
export default {
    ...
    data(){
        return{
            ...
            mapControl:{
                ...
                mapContainerId:'pointMap',
            },
            pageTree: [
                {'id': 1,'treeName': '组织机构', 'treeValue': 'departmentTree'},
            ...
            tableColumns: [
                ...
                {title: '关键点名称', key: 'cloudName',align:'center'},
    ...
    mounted(){
        ...
        this.initMap();
        this.markerHandler = new L.Draw.Marker(this.mapObj.getMap(), {});
    },
    ...
    methods:{
        ...
        saveCloudMap(){
            ...
            this.aoLatLng = this.editCloudMapModal.aoLatLng
            let cLat = this.aoLatLng.lat
            let cLon = this.aoLatLng.lng
        ...
        initPageCfg(){
            ...
            Util.ojax.post(GetPageCfgUrl, {menuId: menuId, roleId: roleId}).then(function (res) {
                ...
                for(let i in cfg){
                    if(cfg[i].name == 'updatePoint'){
                        self.monitorBtn.editBtn = true
                    }
                    if(cfg[i].name == 'insertPoint'){
                        self.monitorBtn.saveBtn = true
                    }
                    if(cfg[i].name == 'deletePoint'){
        ...
        createdCallBack:function (e) {
            ...
            this.aoLatLng = [
                {lat: oLatLng.lat, lng: oLatLng.lng}
            ]
            this.getCenterPoi(oLatLng.lng + ',' + oLatLng.lat) //转化实时地址
            this.isShow = true
            this.modalTitle = '新增关键点'
        },
        // 初始化地图
        initMap() {
        ...
        initCloudMapTree(){
            let self = this
            Util.ojax.post(GetCloudMapTree, {cloudTypeId: 1}).then(function (res) {
                if (res.data.code == 0) {
