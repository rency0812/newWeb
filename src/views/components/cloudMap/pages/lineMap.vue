    ...
    <div class="layout-table">
        ...
        <Split v-model="panelSplit">
            <div slot="left" class="layout-tree-content">
                ...
                <!--&lt;!&ndash;@on-click="addCloudMap"&ndash;&gt;-->
...
import saveWnd from '../modal/save-line'
...
export default {
    ...
    data(){
        return{
            ...
            mapControl:{
                ...
                mapContainerId:'lineMap',
            },
            pageTree: [
                {'id': 1,'treeName': '组织机构', 'treeValue': 'departmentTree'},
                {'id': 2,'treeName': '线路', 'treeValue': 'railTree'}
            ],
            buttonGroup: [
                {
                    id:1,
            ...
            tableColumns: [
                ...
                {title: '线路名称', key: 'cloudName',align:'center'},
    ...
    mounted(){
        ...
        this.initMap();
        this.polyLineHandler = new L.Draw.Polyline(this.mapObj.getMap(), {});
    },
    ...
    methods:{
        ...
        initPageCfg(){
            ...
            Util.ojax.post(GetPageCfgUrl, {menuId: menuId, roleId: roleId}).then(function (res) {
                ...
                for(let i in cfg){
                    if(cfg[i].name == 'updateLine'){
                        self.monitorBtn.editBtn = true
                    }
                    if(cfg[i].name == 'insertLine'){
                        self.monitorBtn.saveBtn = true
                    }
                    if(cfg[i].name == 'deleteLine'){
        ...
        initMap() {
            ...
            // self.mapObj.loadMap(window.innerWidth, window.innerHeight);
        ...
        initCloudMapTree(){
            let self = this
            Util.ojax.post(GetCloudMapTree, {cloudTypeId: 2}).then(function (res) {
                if (res.data.code == 0) {
