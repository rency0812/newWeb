		...
		init : function(options, name) {
			...
			var dtGridObject = {
				...
				constructGrid : function(){
					...
					if(dtGridReflectionObj.exhibitDatas!=null){
						...
						for(var i=0; i<dtGridReflectionObj.exhibitDatas.length; i++){
							...
							if(dtGridReflectionObj.option.check){
							    gridContent += '	<td class="check-column ec-text-center"><input type="checkbox" dataNo="' + i + '" id="dt_grid_' + dtGridReflectionObj.option.id + '_check_' + i + '" class="dt-grid-check" value="' + i + '"></td>';
							}
							var columns = dtGridReflectionObj.option.columns;
							...
							gridContent += '	</tr>';
							gridContent += '	<tr id="dt_grid_'+dtGridReflectionObj.option.id+'_extra_columns_'+i+'" class="dt-grid-extra-columns ec-hide">';
					...
					$('#dt_grid_'+gridId+' .extra-column-event').click(function(e){
						...
						if ($('#dt_grid_' + gridId + '_extra_columns_' + dataNo).hasClass('ec-hide')) {
						    $('#dt_grid_' + gridId + '_extra_columns_' + dataNo).removeClass('ec-hide');
						    ...
						    $('#dt_grid_' + gridId + '_extra_columns_' + dataNo).addClass('ec-hide');
				...
				constructGridPageBar : function(){
					...
					if(dtGridReflectionObj.exhibitDatas==null||dtGridReflectionObj.exhibitDatas.length<=0){
						...
						if(typeof dtGridReflectionObj.option.pageSizeLimit == 'object'){
						    pageSizeElement += '<select id="dt_grid_change_page_size_' + dtGridReflectionObj.option.id + '" name="dt_grid_change_page_size_' + dtGridReflectionObj.option.id + '" class="ec-form-field ec-radius ec-input-sm  change-page-size">';
							//整合可用页码
					...
					operations.className = 'ec-pagination ec-pagination-sm dt-grid-pager-button';
					...
					if(dtGridReflectionObj.exhibitDatas!=null&&dtGridReflectionObj.exhibitDatas.length>0){
						...
						if(pageCount<=5){
							for(var i=1; i<=pageCount; i++){
								...
								goPageBtn.className = i==nowPage?'ec-active':'';
						...
						showPageText.className = 'ec-active';
				...
				constructGridToolBar : function(){
					...
					var dtGridReflectionObj = this;
					//清空工具条
					$('#' + dtGridReflectionObj.option.toolbarContainer).html('<span class="ec-pagination ec-pagination-sm dt-grid-tools"></span><span class="dt-grid-pager"></span>');
					//如果为空则不做任何操作
				...
				faseQuery : function(){
					...
					if(dtGridReflectionObj.init.fastQueryWindowIsInit){
					    $('#faseQuery_' + dtGridReflectionObj.option.id + 'Modal').modal();
						return;
					}
					var content = '';
					...
					for(var j=0; j<dtGridReflectionObj.option.columns.length; j++){
						...
						if(column.fastQuery==true){
							...
							content += '	<label class="ec-ul-sm-3 control-label text-right">'+column.title+'：</label>';
							if(column.fastQueryType=='range'){
							    content += '<div class="ec-ul-sm-4">';
								if(column.type=='date'){
									...
									content += '	<div class="input-group-addon"><i class="ec-icon-calendar"></i></div>';
									content += '</div>';
								}else{
...
//					content += '		<li><a href="#advance_query_plan_'+dtGridReflectionObj.option.id+'" role="tab" data-toggle="tab">'+$.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.plan.title+'</a></li>';
					content += '		<li class="ec-active"><a href="#advance_query_condition_' + dtGridReflectionObj.option.id + '" role="tab" data-toggle="tab">' + $.fn.DtGrid.lang[dtGridReflectionObj.option.lang].advanceQuery.condition.title + '</a></li>';
...
//					content += '			</div>';
//					content += '		</div>';
					content += '		<div class="tab-pane ec-active" id="advance_query_condition_' + dtGridReflectionObj.option.id + '">';
					content += '			<div class="panel panel-default">';
...
//					});
				...
				exportFile : function(exportType){
					...
					$('#dt_grid_export_execute_'+exportType+'_'+dtGridReflectionObj.option.id).click(function(){
						...
						exportFormContainer.className = 'ec-hide';
		...
		tools : {
			...
			getWindowStart : function(id, title){
				var content = '';
				content += '<div class="ec-popup modal fade" id="'+id+'Modal" tabindex="-1" role="dialog" aria-labelledby="'+id+'ModalLabel" aria-hidden="true">';
				content += '	<div class="ec-modal-dialog">';

				content += '			<div class="ec-modal-hd">';
				content +=    				 title ;
				content += '				<a href="javascript: void(0)" class="ec-close ec-close-spin" data-ec-modal-close>×</a>';
...
(function($) {
	$.fn.DtGrid.defaultOptions = {
		grid : {
			...
			tableStyle : '',
			tableClass: 'ec-table ec-table-bordered ec-table-hover ec-table-responsive',
			showHeader : true,
...
(function ($) {
    ...
    $.fn.DtGrid.lang['zh-cn'] = {
        ...
        buttons: {
            close: '<i class="ec-icon-times"></i>&nbsp;&nbsp;关闭'
        },
        extraColumn: {
            open: '<i class="ec-icon-plus"></i>',
            close: '<i class="ec-icon-minus"></i>'
        },
        sortColumn: {
            asc: '<i class="ec-icon-sort-asc"></i>',
            desc: '<i class="ec-icon-sort-desc"></i>'
        },
        toolbar: {
            refresh: '<i class="ec-icon-refresh"></i>',
            refreshTitle: '刷新数据',
            faseQuery: '<i class="ec-icon-search"></i>',
            faseQueryTitle: '快速查询',
            advanceQuery: '<i class="ec-icon-filter"></i>',
            advanceQueryTitle: '高级查询',
            exportExcel: '<i class="ec-icon-file-excel-o"></i>',
            exportExcelTitle: '导出EXCEL文件',
            exportCsv: '<i class="ec-icon-file-o"></i>',
            exportCsvTitle: '导出CSV文件',
            exportPdf: '<i class="ec-icon-file-pdf-o"></i>',
            exportPdfTitle: '导出PDF文件',
            exportTxt: '<i class="ec-icon-file-o"></i>',
            exportTxtTitle: '导出TXT文件',
            print: '<i class="ec-icon-print"></i>',
            printTitle: '打印'
        },
        pageInfo: {
            nothing: '无查询记录...',
            info: '查询出 {recordCount} 条记录，每页 {pageSize} 条，共  {pageCount} 页',
            firstPage: '<i class="ec-icon-angle-double-left"></i>',
            prevPage: '<i class="ec-icon-angle-left"></i>',
            nextPage: '<i class="ec-icon-angle-right"></i>',
            lastPage: '<i class="ec-icon-angle-double-right"></i>',
            firstPageTitle: '第一页',
            prevPageTitle: '上一页',
            nextPageTitle: '下一页',
            lastPageTitle: '最后一页',
        ...
        fastQuery: {
            title: '<i class="ec-icon-search"></i>&nbsp;&nbsp;快速查询',
            ...
            inputEnd: '请输入结束',
            buttons: {
                reset: '<i class="ec-icon-reply"></i>&nbsp;&nbsp;参数重置',
                query: '<i class="ec-icon-search"></i>&nbsp;&nbsp;执行查询'
            }
        },
        advanceQuery: {
            title: '<i class="ec-icon-search"></i>&nbsp;&nbsp;高级查询',
            plan: {
                title: '查询方案',
                ...
                buttons: {
                    addAdvanceQuery: '<i class="ec-icon-plus"></i>&nbsp;&nbsp;新增',
                    addAdvanceQueryTitle: '新增高级查询方案',
                    editAdvanceQuery: '<i class="ec-icon-pencil"></i>&nbsp;&nbsp;编辑',
                    editAdvanceQueryTitle: '编辑高级查询方案',
                    copyAdvanceQuery: '<i class="ec-icon-copy"></i>&nbsp;&nbsp;复制',
                    copyAdvanceQueryTitle: '复制高级查询方案',
                    deleteAdvanceQuery: '<i class="ec-icon-trash-o"></i>&nbsp;&nbsp;删除',
            ...
            condition: {
                ...
                table: {
                    ...
                    logic: '逻辑',
                    buttons: {
                        up: '<i class="ec-icon-arrow-circle-up"></i>',
                        upTitle: '条件上移',
                        down: '<i class="ec-icon-arrow-circle-down"></i>',
                        downTitle: '条件下移',
                        'delete': '<i class="ec-icon-times"></i>',
                        deleteTitle: '删除条件'
                    }
                },
                buttons: {
                    insert: '<i class="ec-icon-plus"></i>&nbsp;&nbsp;新增一行',
                    insertTitle: '新增一行',
                    clear: '<i class="ec-icon-eraser"></i>&nbsp;&nbsp;清空条件',
                    clearTitle: '清空条件'
                },
                errors: {
        ...
        print: {
            title: '<i class="ec-icon-print"></i>&nbsp;&nbsp;打印选项',
            table: {
                column: '列',
                operation: '操作',
                buttons: {
                    up: '<i class="ec-icon-arrow-circle-up"></i>&nbsp;&nbsp;上移',
                    upTitle: '上移',
                    down: '<i class="ec-icon-arrow-circle-down"></i>&nbsp;&nbsp;下移',
                    downTitle: '下移'
                }
            },
            buttons: {
                print: '<i class="ec-icon-print"></i>&nbsp;&nbsp;执行打印'
            }
        },
        'export': {
            excel: {
                title: '<i class="ec-icon-download"></i>&nbsp;&nbsp;导出EXCEL选项',
                exportType: {
                    title: '导出方式：',
                    now: '导出当前数据',
                    all: '导出全部数据'
                ...
                buttons: {
                    'export': '<i class="ec-icon-print"></i>&nbsp;&nbsp;执行导出EXCEL文件'
                }
            },
            csv: {
                title: '<i class="ec-icon-download"></i>&nbsp;&nbsp;导出CSV选项',
                exportType: {
                    title: '导出方式：',
                    now: '导出当前数据',
                    all: '导出全部数据'
                ...
                buttons: {
                    'export': '<i class="ec-icon-print"></i>&nbsp;&nbsp;执行导出CSV文件'
                }
            },
            pdf: {
                title: '<i class="ec-icon-download"></i>&nbsp;&nbsp;导出PDF选项',
                exportType: {
                    title: '导出方式：',
                    now: '导出当前数据',
                    all: '导出全部数据'
                ...
                buttons: {
                    'export': '<i class="ec-icon-print"></i>&nbsp;&nbsp;执行导出PDF文件'
                }
            },
            txt: {
                title: '<i class="ec-icon-download"></i>&nbsp;&nbsp;导出TXT选项',
                exportType: {
                    title: '导出方式：',
                    now: '导出当前数据',
                    all: '导出全部数据'
                ...
                buttons: {
                    'export': '<i class="ec-icon-print"></i>&nbsp;&nbsp;执行导出TXT文件'
                }
            }
        }
    };
